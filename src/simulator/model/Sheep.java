...
/*5*/		if (this.state != State.DEAD) {
				...
				this.energy = Utils.constrainValueInRange(this.energy + food, 0.0, MAX_ENERGY); 
			}
		}
	}
	...
	private void posIsOut() {
		...
		if (posX < 0 || posX >= maxWidth || posY < 0 || posY >= maxHeight) {
			while (posX >= maxWidth) posX = (posX - maxWidth);  
			while (posX < 0) posX = (posX + maxWidth);  
			while (posY >= maxHeight) posY = (posY - maxHeight);  
			while (posY < 0) posY = (posY + maxHeight);
	...
	private void stateNormal(Double dt) {
		if (this.pos.distanceTo(this.dest) < 8.0) {  // COLLISION_RANGE la distancia al dest < 8 -->cambiar de dest
		...
		if (this.dangerSource == null) {
			dangerSource = dangerStrategy.select(this, regionMngr.getAnimalsInRange(this, (Animal a) -> a.getDiet().equals(Diet.CARNIVORE)));	
		}
		
		if (this.dangerSource != null) {  
			setState(State.DANGER); 
		}
		else if(this.dangerSource == null && this.desire > 65){
			setState(State.MATE);
		}
	}
