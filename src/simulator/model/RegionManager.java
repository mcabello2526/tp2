...
public class RegionManager implements AnimalMapView {
	...
	public RegionManager(int cols, int rows, int width, int height) {
		if (cols <= 0 || rows <= 0 || width <= 0 || height <= 0) {
			throw new IllegalArgumentException(
					"Invalid map dimensions. Columns, rows, width and height need to be a value greater than 0 (> 0");
		}
		this.cols = cols;
		this.rows = rows;
		this.width = width;
	...
	void setRegion(int row, int col, Region r) {
		...
		for (Animal a : region.getAnimals()) {  // me da una lista unmodifiable 
			r.addAnimal(a);
	...
	@Override  // por parte del FoodSupplier 
	...
	private class RegionIterator implements Iterator<MapInfo.RegionData>{  // clase anidada
		
		private int currRows = 0; 
		private int currCols = 0; 

		@Override
		public boolean hasNext() {
			return currRows < rows && currCols < cols;
		}

		@Override
		public RegionData next() {
			// if (!hasNext()) 
			MapInfo.RegionData regionData = new MapInfo.RegionData(currRows, currCols, regions[currRows][currCols] ); 
			currCols ++; 
			if (currCols >= cols) {
				currCols = 0; 
				currRows++; 
