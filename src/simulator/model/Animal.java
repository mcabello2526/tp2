...
public abstract class Animal implements AnimalInfo, Entity {
	...
	final static double MINIMUM_VALUE = 0.0;
	...
	protected Animal(String geneticCode, Diet diet, double sightRange, double initSpeed, SelectionStrategy mateStrategy,
			Vector2D pos) throws IllegalArgumentException {

		if (geneticCode == null || geneticCode.equals("")) {
			throw new IllegalArgumentException("Invalid genetic code. It cannot be empty.");
		}
		if (initSpeed < 0 || sightRange < 0) {
			throw new IllegalArgumentException("Invalid intial speed. It cannot be below 0");
		}
		if (mateStrategy == null) {
			throw new IllegalArgumentException("Invalid mate strategy. It cannot be null");
		}
	...
	protected Animal(Animal p1, Animal p2) {
		if (p1 == null || p2 == null) {
			throw new IllegalArgumentException("This Animal value is null");
		}
		dest = null;
		mateTarget = null;
		baby = null;
		...
		speed = Utils.getRandomizedParameter((p1.getSpeed() + p2.getSpeed()) / 2, 0.2); /* MUTATION_TOLERANCE */
	}
