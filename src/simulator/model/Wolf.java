...
public class Wolf extends Animal {
	...
	private void die() {
		if (this.energy <= MINIMUM_VALUE || this.age > MAX_AGE_WOLF) {
			setState(State.DEAD);
		}
	}
	...
	private void stateNormal(Double dt) {
		...
		this.move(speed * dt * Math.exp((energy - MAX_ENERGY) * HUNGER_DECAY_EXP_FACTOR));
		this.age += dt;
		this.energy = Utils.constrainValueInRange((this.energy - (FOOD_DROP_RATE_WOLF * dt)), MINIMUM_VALUE,
				MAX_ENERGY);
		this.desire = Utils.constrainValueInRange((this.desire + (DESIRE_INCREASE_RATE_WOLF * dt)), MINIMUM_VALUE,
				MAX_DESIRE);

		if (this.energy < FOOD_THRESHOLD_WOLF) {
	...
	private void stateHunger(Double dt) {
		if (this.huntTarget == null || this.huntTarget != null && (this.huntTarget.getState().equals(State.DEAD)
				|| this.pos.distanceTo(this.huntTarget.getPosition()) > this.sightRange)) {
			huntTarget = huntingStrategy.select(this,
					regionMngr.getAnimalsInRange(this, (Animal a) -> a.getDiet().equals(Diet.HERBIVORE)));
		...
		if (this.huntTarget == null) {
			...
			this.move(BOOST_FACTOR_WOLF * speed * dt * Math.exp((energy - MAX_ENERGY) * HUNGER_DECAY_EXP_FACTOR));
			this.age += dt;
			this.energy = Utils.constrainValueInRange(
					this.energy - (FOOD_DROP_RATE_WOLF * FOOD_DROP_BOOST_FACTOR_WOLF * dt), MINIMUM_VALUE, MAX_ENERGY);
			this.desire = Utils.constrainValueInRange(this.desire + (DESIRE_INCREASE_RATE_WOLF * dt), MINIMUM_VALUE,
					MAX_DESIRE);
			...
			if (this.pos.distanceTo(this.huntTarget.getPosition()) < COLLISION_RANGE) {
				...
				huntTarget = null;
				this.energy = Utils.constrainValueInRange(this.energy + FOOD_EAT_VALUE_WOLF, MINIMUM_VALUE, MAX_ENERGY);
			}
		}
	...
	private void stateMate(double dt) {
		if (this.mateTarget != null
				&& (this.state == State.DEAD || this.pos.distanceTo(this.mateTarget.getPosition()) > this.sightRange)) {
		...
		if (this.mateTarget == null) {
			...
			if (mateTarget == null) {
				...
				this.energy = Utils.constrainValueInRange((this.energy - (FOOD_DROP_RATE_WOLF * dt)), MINIMUM_VALUE,
						MAX_ENERGY);
				this.desire = Utils.constrainValueInRange((this.desire + (DESIRE_INCREASE_RATE_WOLF * dt)),
			...
			if (this.pos.distanceTo(this.mateTarget.getPosition()) < COLLISION_RANGE) {
				...
				this.energy = Utils.constrainValueInRange((this.energy - (FOOD_DROP_DESIRE_WOLF)), MINIMUM_VALUE,
						MAX_ENERGY);
				this.mateTarget = null;
