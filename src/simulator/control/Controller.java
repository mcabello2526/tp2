...
public class Controller {
	...
	public Controller(Simulator sim) {
		if (sim == null) {
		 throw new IllegalArgumentException("The simulator cannot be null"); 
		}else {
			this.sim = sim;
		}
		
	}
// clave : Animal y valores : regions  --> cada elemento JSONObject
	...
	private List<ObjInfo> toAnimalsInfo(List<? extends AnimalInfo> animals) {
	    return animals.stream()
	        .map(a -> new ObjInfo(a.getGeneticCode(), 
	                              (int) a.getPosition().getX(), 
	                              (int) a.getPosition().getY(), 
	                              (int) Math.round(a.getAge()) + 2))
	        .collect(Collectors.toList()); // O .toList() en versiones modernas de Java
	}
	*/
	...
	public void setRegions(JSONObject rs) {
		...
		regionsArray = rs.getJSONArray("regions");
